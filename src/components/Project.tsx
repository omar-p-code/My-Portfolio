...
function project({ project }: { project: Project }) {
   ...
   return (
      ...
      active:-translate-y-2 active:border-(--secondary) active:scale-95 uppercase">
         ...
         <img src={cover} alt={title} className="w-full h-full absolute object-cover opacity-40 md:opacity-15 object-center -z-30 " />


         <div className="overlay hover-show z-30 bg-(--card)/30 absolute top-0 left-0 w-full h-full flex items-center justify-center md:opacity-0 max-lg:backdrop-blur-xs text-lg font-bold text-(--secondary) transition-opacity duration-400 cursor-pointer flex-col gap-6 p-5">
